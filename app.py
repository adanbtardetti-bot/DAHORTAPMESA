            ...
            with st.expander(f"👤 {row['cliente']} | {stpg}", expanded=True):
                ...
                for i, it in enumerate(itens_m):
                    ...
                    if str(it['tipo']).upper() == "KG":
                        # AQUI É ONDE O PESO/VALOR É LANÇADO
    ...
    if not df_pedidos.empty and "status" in df_pedidos.columns:
        ...
        for _, row in hist.iterrows():
            ...
            with st.expander("📋 Detalhes"):
                # CORREÇÃO: MOSTRANDO VALOR INDIVIDUAL NO HISTÓRICO
        ...
        for _, r in df_res.iterrows():
            try:
                for it in json.loads(r['itens']):
                    ...
                    # CORREÇÃO: ACUMULANDO PESO/QTD E VALOR REAL
                    res[n]["qtd"] += it['qtd']
                    res[n]["val"] += parse_float(it.get('subtotal', 0))
            except: continue
        # MOSTRANDO PESO/QTD E VALOR NA TABELA
        tab_dados = [{"Produto": k, "Qtd/Peso": v["qtd"], "Total (R$)": f"{v['val']:.2f}"} for k, v in res.items()]
