...
# Estilos para evitar quebras no celular e formatar a nova tela
st.markdown('<style>div[data-testid="stColumn"]{display:flex;align-items:center;} .btn-whatsapp{background-color:#25d366;color:white;padding:15px;border-radius:10px;text-align:center;text-decoration:none;display:block;font-weight:bold;margin-top:10px;} .card-montagem {border:1px solid #2e7d32; padding:8px; border-radius:10px; background-color:#0e1117; margin-bottom:5px;}</style>', unsafe_allow_html=True)
...
# --- ALTERAÇÃO AQUI: DEFININDO AS 3 ABAS ---
...
with aba3:
    ...
    if not df_m.empty and 'status' in df_m.columns:
        ...
        for idx, row in pend_m.iterrows():
            with st.container():
                st.markdown(f'<div class="card-montagem"><b>👤 {row["cliente"]}</b> | 📍 {row["endereco"]}<br><small>💬 {row.get("obs", "")}</small></div>', unsafe_allow_html=True)
                ...
                for i, item in enumerate(itens_m):
                    ...
                    if str(item.get('tipo', '')).upper() == "KG":
                        ...
                        c_it.write(f"✅ {item.get('qtd', 0)}x {item['nome']}")
                ...
                b2.markdown(f'<a href="{l_p}"><button style="width:100%; height:
