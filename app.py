        ...
        for _, row in pend_m.iterrows():
            ...
            with st.expander(f"👤 {row['cliente']} | {stpg}", expanded=True):
                ...
                if st.session_state[edit_key]:
                    # Interface de edição restaurada
                    ...
                    lista_prod = ["-- Selecionar --"] + sorted(df_produtos[df_produtos['status'].astype(str).str.lower() == "ativo"]['nome'].tolist())
                    p_sel = c_add_p.selectbox("Produto", lista_prod, key=f"sel_e_{row['id']}", label_visibility="collapsed")
                    q_sel = c_add_q.number_input("Qtd", 1, step=1, key=f"qtd_e_{row['id']}", label_visibility="collapsed")
                    
                    if c_add_b.button("Adicionar", key=f"btn_e_{row['id']}"):
                        if p_sel != "-- Selecionar --":
                            its = json.loads(row['itens'])
                            p_inf = df_produtos[df_produtos['nome'] == p_sel].iloc[0]
                            its.append({"nome": p_sel, "qtd": q_sel, "preco": parse_float(p_inf['preco']), "tipo": p_inf['tipo'], "subtotal": 0.0 if p_inf['tipo'] == "KG" else (q_sel * parse_float(p_inf['preco']))})
                            df_f = ler_aba("Pedidos", 0); df_f.loc[df_f["id"].astype(str) == str(row["id"]), "itens"] = json.dumps(its); salvar_aba("Pedidos", df_f); st.session_state.reload_pedidos = True; st.rerun()
                    ...
                    for i, it in enumerate(itens_edit):
                        c_n, c_q, c_d = st.columns([3, 1, 0.5])
                        c_n.markdown(f"**{it['nome']}**")
                        nq = c_q.number_input("Q", value=int(it['qtd']), min_value=0, key=f"ed_q_{row['id']}_{i}", label_visibility="collapsed")
                        if c_d.button("🗑️", key=f"del_i_{row['id']}_{i}"): nq = 0
                        if nq > 0:
                            it['qtd'] = nq
                            if str(it['tipo']).upper() != "KG": it['subtotal'] = nq * parse_float(it['preco'])
                            novos_itens_f.append(it); total_ed += parse_float(it['subtotal'])
                    
                    st.write(f"**Total parcial: R$ {total_ed:.2f}**")
                    if st.button("💾 SALVAR", key=f"save_ed_{row['id']}", type="primary", use_container_width=True):
                        df_f = ler_aba("Pedidos", 0); df_f.loc[df_f["id"].astype(str) == str(row["id"]), ["cliente", "endereco", "itens", "total"]] = [novo_nome, novo_end, json.dumps(novos_itens_f), total_ed]; salvar_aba("Pedidos", df_f); st.session_state[edit_key] = False; st.session_state.reload_pedidos = True; st.rerun()
                    if st.button("Cancelar", key=f"cn_ed_{row['id']}", use_container_width=True): st.session_state[edit_key] = False; st.rerun()
                    ...
                    for i, it in enumerate(itens_m):
                        ...
                        if str(it['tipo']).upper() == "KG":
                            ...
                            it['subtotal'] = v_dig
                            if v_dig > 0 and parse_float(it.get('preco', 0)) > 0: it['qtd'] = round(v_dig / parse_float(it['preco']), 3)
                    ...
                    # BOTÕES ORIGINAIS COM LÁPIS SIMPLES
                    c_ok, c_pg, c_ed, c_pr, c_del = st.columns([1, 1, 0.5, 0.5, 0.5])
                    
                    if c_ok.button("📦 OK", key=f"ok_{row['id']}"):
                        df_f = ler_aba("Pedidos", 0); idx_l = df_f.index[df_f["id"].astype(str) == str(row["id"])].tolist()
                        if idx_l:
                    ...
                    if c_ed.button("✏️", key=f"btn_e_{row['id']}"): # Botão de lápis simplificado
...
with aba5:
    ...
    if not df_pedidos.empty:
        agora_br = obter_data_br()
        if menu == "Dia": gerar_tabela_fin(df_pedidos[df_pedidos["data"] == agora_br.strftime("%d/%m/%Y")], "RELATÓRIO DO DIA")
        elif menu == "Período":
            c1, c2 = st.columns(2)
